...
Public DECK1NAME As String
Public DECK2NAME As String
Public EFXNAME As String
Public RITMNAME As String
...
Public Function PLAYEROPEN(hWnd As Long, AliasName As String, Filename As String, typeDevice As String) As String
'Callig PLAYEROPEN will open the multimedia file
'Parameters
'hWnd
...
'PLAYERCLOSE or CloseAll When you will end your program or you
...
If Not dwReturn = 0 Then  'not success
    ...
    PLAYEROPEN = ret: Exit Function
End If

'Success
PLAYEROPEN = "Success"
Select Case AliasName
Case Is = "SONG"
 If PLAYEROPEN = "Success" Then SONGNAME = Filename Else SONGNAME = ""
Case Is = "DECK1"
 If PLAYEROPEN = "Success" Then DECK1NAME = Filename Else DECK1NAME = ""
Case Is = "DECK2"
 If PLAYEROPEN = "Success" Then DECK2NAME = Filename Else DECK2NAME = ""
Case Is = "EFX"
 If PLAYEROPEN = "Success" Then EFXNAME = Filename Else EFXNAME = ""
Case Is = "RITM"
 If PLAYEROPEN = "Success" Then RITMNAME = Filename Else RITMNAME = ""
End Select
End Function

Public Function PLAYERPLAY(AliasName As String, from_where As String, to_where As String) As String
'Calling PLAYERPLAY will playing the multimedia file.
'Parameters

'AliasName
'[in]Specifies name alias name which you want play it
'Note : you must let this parameter the alias which you
'used it PLAYEROPEN Function or this function not Success
...
If to_where = vbNullString Then to_where = PLAYERDURATION(AliasName)
...
Public Function PLAYERCLOSE(AliasName As String) As String
'Calling PLAYERCLOSE will close the multimedia file
...
'you must call this function if you called PLAYEROPEN
...
PLAYERCLOSE = "Success"
If AliasName = "DECK1" And PLAYERCLOSE = "Success" Then DECK1NAME = ""
If AliasName = "DECK2" And PLAYERCLOSE = "Success" Then DECK2NAME = ""
If AliasName = "EFX" And PLAYERCLOSE = "Success" Then EFXNAME = ""
If AliasName = "RITM" And PLAYERCLOSE = "Success" Then RITMNAME = ""
End Function

Public Function PLAYERPAUSE(AliasName As String) As String
'Calling PLAYERPAUSE will pause the multimedia file
...
Public Function PLAYERSTOP(AliasName As String) As String
'Calling PLAYERSTOP will Stop the multimedia file
...
Public Function PLAYERRESUME(AliasName As String) As String
'Calling PLAYERRESUME will Resume the multimedia file
'note: if you paused or stopped the file call this function to Continue
'( don't call PLAYERPLAY function to Continue)
...
'Success
PLAYERRESUME = "Success"
End Function

Public Function PLAYERSTATUS(AliasName As String) As String
'Calling Function PLAYERSTATUS will tell if the multimedia file
...
'Result = PLAYERSTATUS("aliasname")'alias name for e.g. movie
...
For i = 1 To Len(Status)
    ...
    PLAYERSTATUS = PLAYERSTATUS + CharA
Next i
End Function

Public Function PLAYERDURATION(AliasName As String) As Long
'Calling PLAYERDURATION will Get the Total frames for
'the multimedia file
...
Public Function PLAYERSETPOS(AliasName As String, to_where As Long) As String
'Calling PLAYERSETPOS will seek (change the position)for
'the multimedia file
...
'Success
PLAYERSETPOS = "Success"
End Function

Public Function PLAYERGETPOS(AliasName As String) As Long
'Calling Function PLAYERGETPOS will get the current frame
...
TotalFrames = PLAYERDURATION(AliasName)
currframe = PLAYERGETPOS(AliasName)
...
'Function PLAYEROPEN (directly)before resize the movie.
...
End If
End Function
Public Function PLAYERMUTE(AliasName As String, Channel As String, OnOrOFF As String) As String
'Callig PLAYERMUTE will make controls for channels audio (left and right)
...
Public Function PLAYERSETVOLUME(AliasName As String, Channel As String, VolumeValue As Long) As String
'Callig PLAYERSETVOLUME will make control for volume channels
...
'Success
PLAYERSETVOLUME = "Success"
End Function


Public Function PLAYERGETVOLUME(AliasName As String, Channel As String) As Long
'Callig PLAYERGETVOLUME will get the volume for Specified channels (left or right) or both channels
...
'Success
PLAYERGETVOLUME = Val(Volume) / 10
End Function

Public Function PLAYERSETRATE(AliasName As String, RateValue As Long) As String
'Callig PLAYERSETRATE will increase or decrease speed playing for Multimedia file
...
'Success
PLAYERSETRATE = "Success"
End Function

Public Function PLAYERGETRATE(AliasName As String) As Long
'Callig PLAYERGETRATE will get current rate for Multimedia file
...
'If AreMultimediaAtEnd("aliasname") = True Then' alias name for e.g.:"movie"
    ...
    'Result = PLAYERPLAY("aliasname",txtFrom, TxtTo)

    ''or you have choice to close this File and open
    ''another file and play it( this if had a list of files):
    
    'Dim Result As String
    'Result = PLAYERCLOSE("aliasname")
    'Result = PLAYEROPEN(FrameVideo.hwnd,"aliasname", filename, typeDevice) 'call now function PLAYEROPEN
    'Result = PLAYERPLAY("aliasname",txtFrom, TxtTo)
...
If lastFrame = 0 Then lastFrame = PLAYERDURATION(AliasName)

currpos = Val(PLAYERGETPOS(AliasName))
...
'2-keep in your mind if you want to use this function call it after calling PLAYEROPEN function not else.
...
If last_frame = vbNullString Then last_frame = PLAYERDURATION(AliasName)
...
Dim currpos As Long
Dim Result As String
currpos = Val(PLAYERGETPOS(glo_AliasName))
...
If Val(glo_to) = currpos Or (Val(glo_to) - 1) < currpos Then
    Result = PLAYERPLAY(glo_AliasName, Str(glo_from), Str(glo_to))
...
Public Function WAVEINFO(Filename As String) As String
 ...
 Dim beg As Byte
  ...
  Else
    ...
    WAVEINFO = WAVEINFO + Trim(Str(Asc((Mid(Buf, 25, 1))))) & " KHz, "
    'The 35 byte in a wave file determines if the file is 16bit(16 ascii) or 8bit(8 ascii)
    WAVEINFO = WAVEINFO + Trim(Str(Asc((Mid(Buf, 35, 1))))) & " Bit."
  End If
End Function

Public Function issound() As Boolean
Dim i As Long
